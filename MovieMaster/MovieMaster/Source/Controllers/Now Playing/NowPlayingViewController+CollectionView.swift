//
//  NowPlayingViewController+CollectionView.swift
//  MovieMaster
//
...
extension NowPlayingViewController: UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate {
    ...
    internal func setupCollectionViewItemSize() {
        if collectionViewFlowLayout == nil {
            ...
            nowPlayingCollectionView.setCollectionViewLayout(collectionViewFlowLayout, animated: true)
        }
    }
        ...
        if !isDelegateSet {
            ...
            if offSetY > contentHeight - scrollView.frame.size.height * Constants.leadingScreensForBatching {
                ...
                if (!fetchingMore && !endReached && (movieCurrentPage + 1 <= movieTotalPages)) {
                    fetchNowPlaying(page: movieCurrentPage + 1)
                }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: Constants.StringIdentifier.CollectionViewCell.cell, for: indexPath) as? MovieCollectionViewCell, let nowPlayingMovies = viewModel?.getMovies() {
            cell.movieCover = nowPlayingMovies[indexPath.row]
            return cell
        }
    ...
    func registerNib(_ nibName: String = Constants.StringIdentifier.CollectionViewCell.movieCollectionViewCell) {
        ...
        nowPlayingCollectionView.register(nibName, forCellWithReuseIdentifier: Constants.StringIdentifier.CollectionViewCell.cell)
    }
