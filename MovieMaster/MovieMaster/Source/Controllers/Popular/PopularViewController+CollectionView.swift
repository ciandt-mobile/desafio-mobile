//
//  PopularViewController+CollectionView.swift
//  MovieMaster
//
...
extension PopularViewController: UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate {
        ...
        if !isDelegateSet {
            ...
            if offSetY > contentHeight - scrollView.frame.size.height * Constants.leadingScreensForBatching {
                ...
                if (!fetchingMore && !endReached && (movieCurrentPage + 1 <= movieTotalPages)) {
                    fetchPopular(page: movieCurrentPage + 1)
                }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: Constants.StringIdentifier.CollectionViewCell.cell, for: indexPath) as? MovieCollectionViewCell, let popularMovies = viewModel?.getMovies() {
            cell.movieCover = popularMovies[indexPath.row]
            return cell
        }
    ...
    func registerNib(_ nibName: String = Constants.StringIdentifier.CollectionViewCell.movieCollectionViewCell) {
        let nibName = UINib(nibName: Constants.StringIdentifier.CollectionViewCell.movieCollectionViewCell, bundle: nil)
        popularCollectionView.register(nibName, forCellWithReuseIdentifier: Constants.StringIdentifier.CollectionViewCell.cell)
    }
